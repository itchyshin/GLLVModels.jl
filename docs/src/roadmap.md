...
(`engine = "julia"`). The bridge does not yet cover every R workflow; see
...
after its results, documentation, and tests agree at the stated scope. REML is
Gaussian-only; AI-REML is future work for exact Gaussian cells, not
...
| **v1.0** | True-parity milestone (aspirational) | Targets documented agreement with frozen gllvmTMB 0.7.0 across realistic-size and real-data workflows; it does **not** promise a complete Julia bridge for every R workflow |
