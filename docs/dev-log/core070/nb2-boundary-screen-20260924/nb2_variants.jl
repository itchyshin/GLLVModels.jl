...
datasets = [(s, r) for s in (45, 46, 47) for r in (1.0, 1.5, 2.0)]
push!(datasets, (45, 4.0))   # NATIVE-06 design (Julia 1.10 draw)
...
for (seed, r_true) in datasets
    ...
    θc, llc, cc = nbfit(Y, K, default_start(Y, K, log(10.0)))           # current
    θa, lla, ca = nbfit(Y, K, default_start(Y, K, 0.0))                 # (a) start r = 1
    θb, llb = θc, llc                                                   # (b) restart boundary traits at r = 1
    rb = exp.(θc[p+rr+1:end]); bd = findall(x -> !(1e-6 <= x <= 1e6), rb)
    if !isempty(bd)
        θs = copy(θc); θs[p+rr .+ bd] .= 0.0
        θr, llr, _ = nbfit(Y, K, θs); llr > llb && ((θb, llb) = (θr, llr))
    end
    nb(θ) = count(x -> !(1e-6 <= x <= 1e6), exp.(θ[p+rr+1:end]))
    @printf("seed=%d r=%.1f | R %.4f | current %+.1e (bd %d) | (a) %+.1e (bd %d) | (b) %+.1e (bd %d)\n",
        seed, r_true, Rll, llc - Rll, nb(θc), lla - Rll, nb(θa), llb - Rll, nb(θb))
end
