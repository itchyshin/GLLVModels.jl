...
println("== fit_gllvm_cov(Poisson) : converged flag vs gradient at returned point"); flush(stdout)
for seed in (1, 5, 6)
    rng = MersenneTwister(seed)
    Y, X = sim_pois(rng; lam = seed <= 4 ? 1.0 : 1.8)
    p, n = size(Y); K = 2
    t = @elapsed fit = G.fit_gllvm_cov(Y; family=Poisson(), X=X, K=K, iterations=150)
    ...
    g = fdgrad(f, θ)
    (println("seed=$seed  converged=$(fit.converged)  iters=$(fit.iterations)  loglik=$(round(fit.loglik, digits=3))  max|grad|=$(round(maximum(abs,g), sigdigits=3))  time=$(round(t,digits=1))s"); flush(stdout))
end

println("== fit_zip_gllvm : converged flag vs gradient at returned point"); flush(stdout)
for seed in (1, 5, 6)
    ...
    t = @elapsed fit = G.fit_zip_gllvm(Y; K=K, iterations=150)
