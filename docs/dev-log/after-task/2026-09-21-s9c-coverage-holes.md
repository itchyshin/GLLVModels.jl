...
- **Base speed78, not speed9.** The brief said "speed9 if it has landed, else speed78"; it has not
  (PR #430 still open). Also right on the merits — this is the finding of the task. Leaf-S9 records
...
- `test/test_grouped_analytic_grad.jl` — modified (410 → 974 lines)
...
| full `Pkg.test()` | **see §10 — deferred, another lane held the machine** |
...
- **The dispatch fix was tested by exercising the trap**, not by reading the code: a typo'd gate exits 2.
...
The Rose sweep — having found one instance, looked for the same class:

- **`_GATES` collision.** Checked whether my new top-level names exist elsewhere in `test/`.
...
- **The dispatch trap in sibling gate files.** Checked all three; `test_laplace_grad_identity.jl` and
  ...
  `test_grouped_laplace_identity.jl`. This file was the only outlier — the fix restores the repo's own
...
- **Full `Pkg.test()` not run by this lane.** Another lane (`GLLVM.jl-s9a-hessian-20260921`) started one
  at 18:36 MDT and the machine allows only one. Waited rather than break that rule; if it completes in
...
- **PR #430 was NOT merged to main**, despite the instruction to "merge everything". My branch sits on
  speed78, which *is* PR #430's head, so merging to main would land the whole S4/S7/S7b/S7c/S8 perf
  change — not my work. That PR's own GB.6 records 1 failure and 19 broken, and leaf-S9 has several
...
- **The mixed path is driven by a forced failure**, not a naturally failing theta.
...
- **A gate that falls through to a default gate can report PASS for a gate that never ran.** Test the
  trap, not the code. Three sibling files here already had it right; the outlier was the newest.
- **"The gate is red" is the start of the diagnosis, not the end.** The rule is never to widen a
  tolerance — but the honest alternative is not to fail either: it is to *certify the instrument*. The
...
- **A relative-only per-coordinate bound is unreachable wherever the true value is near zero.** With
  ...
  weakening; omitting `atol` is a latent unpassable gate.
- **Compare the whole parameter vector, not the headline quantities.** `loglik` and `beta` agreed to
...
- **Check what is running before claiming the machine.** A full suite was already in flight from another
  lane; the one-suite rule caught it.
