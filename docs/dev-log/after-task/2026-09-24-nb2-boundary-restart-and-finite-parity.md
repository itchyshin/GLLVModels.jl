...
- **Restart (#477).** `_nb_boundary_restart` in `src/families/grouped_dispersion.jl`, called by
  ...
  the kept run only) and CHANGELOG updated.
...
- **Not in scope:** `fit_nb_gllvm_grouped_cov` and the other grouped families (screen running
  separately; listed in #477).
...
- `test/fixtures/nb2_restart_seed46.toml`, `nb2_restart_seed52.toml`, `nb2_interior_n200_seed46.toml` (new)
...
- `docs/dev-log/core070/nb2-boundary-screen-20260924/` (new: scripts, outputs, README)
...
| `test/test_nb_boundary_restart.jl`, final | 10 of 10 |
| 11 existing test files that use the fitter, final code | all pass: `test_grouped_dispersion` 20, `test_nb_fit` 8, `test_fit_gllvm` 11, `test_aicbic_newfits` 18, `test_unified_api` 24, `test_known_sentinel_defects` 25 + 1 known broken, `test_grouped_hessian_consistency` 23, `test_bridge_grouped_dispersion` 129, `test_bridge_x` 192, `test_confint_family` 326 |
...
- #477: fixed for `fit_nb_gllvm_grouped`; other grouped fitters being screened.
...
- This change does NOT cover: `fit_nb_gllvm_grouped_cov`, other grouped-dispersion families, the
  frozen NATIVE-06 cell, boundary maxima of traits not already at the boundary, or gllvmTMB's own
  stalls.
