...
sizes = [(8, 100), (15, 300)]
for (p, n) in sizes, seed in 1:4
    Y = sim(:pois, seed; p, n);  run("pois-FD p$p n$n s$seed", () -> fit_poisson_gllvm(Y; K = 2, gradient = :finite))
    run("pois-analytic p$p n$n s$seed", () -> fit_poisson_gllvm(Y; K = 2))
    Y = sim(:binom, seed; p, n); run("binom-FD p$p n$n s$seed", () -> fit_binomial_gllvm(Y; K = 2, gradient = :finite))
    Y = sim(:gamma, seed; p, n); run("gammagrp-FD p$p n$n s$seed", () -> GLLVModels.fit_gamma_gllvm_grouped(Y; K = 2, group = repeat(1:2, cld(p,2))[1:p]))
    Y = sim(:gauss, seed; p, n); run("gauss-fwd p$p n$n s$seed", () -> fit_gaussian_gllvm(Y; K = 2))
end
